...
export default async function handler() {
  try {
    const res = await fetch(
      ...
      { headers: { 'User-Agent': 'Mozilla/5.0 (compatible; MortgageAdvisorBot/1.0)', Accept: 'text/html' } }
    );
      ...
      if (match) {
        ...
        if (!isNaN(rate) && rate > 0 && rate < 30) {
          return new Response(
            ...
            { headers: { 'Content-Type': 'application/json', 'Cache-Control': 's-maxage=3600, stale-while-revalidate=86400', 'Access-Control-Allow-Origin': '*' } }
          );
        }
      }
    }
    throw new Error('Could not parse Bank Rate');
  } catch (err) {
    return new Response(
